...
(function(){
  ...
  function makePads(hostId, D){
    ...
    function draw(step){
      ...
      var n = data.force.length;
      [[0, NAVY], [3, GOLD]].forEach(function(spec){
        g.beginPath();
        for (var i = 0; i < n; i++) {
          var x = sx + (n === 1 ? 0 : i/(n-1)) * sw;
          var y = sy + sh - norm(mag(data.force[i], spec[0])) * sh;
    ...
    draw(0);
    return { draw: draw, setData: function(nd){ data = nd; draw(0); } };
  }
  ...
  // swap a plot (and pads) to a new rollout without tearing anything down
  function updateData(pads, trajId, D, small){
    var tr = D.traj, n = tr.length;
    var mkc = D.success ? '#f0f921' : '#cc2222';
    var mkl = D.success ? '#b9821f' : '#7c1414';
    Plotly.restyle(trajId, { x:[cols(tr,0,n-1)], y:[cols(tr,1,n-1)], z:[cols(tr,2,n-1)] }, [0]);
    Plotly.restyle(trajId, { x:[[tr[0][0]]], y:[[tr[0][1]]], z:[[tr[0][2]]], 'line.color':[[0]] }, [1]);
    Plotly.restyle(trajId, { x:[[tr[0][0]]], y:[[tr[0][1]]], z:[[tr[0][2]]] }, [2]);
    Plotly.restyle(trajId, { x:[[tr[0][0]]], y:[[tr[0][1]]], z:[[tr[0][2]]],
                             'marker.color': mkc, 'marker.line.color': mkl }, [3]);
    if (pads) pads.setData(D);
  }
  ...
  window.VIZ = { buildPair: buildPair, sync: sync, step: step, bindSeek: bindSeek, updateData: updateData,
